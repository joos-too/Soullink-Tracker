...
- Tracker items and fossils are stored as one `ItemEntry`/`FossilEntry` per physical item. Identical entries (duplicates are possible for fossils with "Infinite Fossils") are merged only for display via `src/services/items/itemGroups.ts` (item/fossil tracker and search modal); edit mode shows the ungrouped entries. Revived fossils count as used. Item/fossil names, sprites, statuses and group summaries come from `src/services/items/itemDisplay.ts`; both views render cards with `src/components/other/ItemGroupCard.tsx` and `PlayerColumnHeader.tsx`.
- The graveyard and the tracker search modal render linked Pokémon with `src/components/other/PokemonPairCard.tsx`.
